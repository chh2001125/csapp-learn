...
void eval(char *cmdline) 
{
    ...
    int bg;
    char buf[MAXLINE];
    sigset_t mask_chld,prev,mask_all;
    ...
    if(!builtin_cmd(argv)){
       ...
       if((pid = fork()) == 0){
           ...
           if(execve(argv[0],argv,environ) < 0){
               ...
               unix_error("%s: Command not found",cmdline);
           }
       }
       else if(pid < 0)
           unix_error("fork fali");
       int status = bg ? BG : FG;
       sigprocmask(SIG_BLOCK,&mask_all,NULL);
       addjob(jobs,pid,status,buf);
       sigprocmask(SIG_SETMASK,&prev,NULL);
       if(!bg){
           waitfg(pid);
       }
...
{
    if(argv[1] == NULL){
        printf("%s command need a PID or %%jobid argument\n",argv[0]);
        return;
    }
    struct job_t* job;
    pid_t pid;
...
void waitfg(pid_t pid)
{
    sigset_t mask_all,prev,mask_empty;
    sigfillset(&mask_all);
